...
void CupRootNtuple::SetPhoton() {
    // ---> JW (2024.02.19.): Get The number of PMTs from thei file pmtcoordinates
    std::ifstream wherePMT;
    if (getenv("AmoreDATA") != NULL){
        wherePMT.open((G4String(getenv("AmoreDATA")) + "/" + G4String("pmtpos_amore200.dat")).c_str());
    } 
    ...
    int maxODPMTNo = 0; // If outer PMTs are modified, this value should be changed.
    ...
    Int_t n_op_scint       = 0;
    Int_t n_op_scint_test  = 0; // JW (2024.02.19.): NoProcess check
    Int_t n_op_other       = 0; // JW (2024.02.19.): for debugging
    Int_t n_op_reem        = 0; 
    Float_t xx, yy, zz;
    Int_t tag;
    ...
    for (int ipmt = 0; ipmt < theHitPMTCollection.GetEntries(); ipmt++) {
        ...
        for (int i = 0; i < a_pmt->GetEntries(); i++) {
            ...
            if (tag == -1) n_op_other++; // JW (2024.02.19.): for debugging
            if (tag == 1) n_op_cerenkov++;
            if (tag == 2) n_op_scint++;
            if (tag == 3) n_op_reem++;
            if (tag == 4) n_op_scint_test++; // JW (2024.02.19.): NoProcess check
    ...
    if (pmtsd_inner) {
        ...
        pmtsd_inner->SetNtotPmts(maxIDPMTNo); // JW (2024.02.19.): Set the number of PMTs
        pmtsd_inner->SetNhitPmts(nhitPmtInner);
        pmtsd_inner->SetNhits(nhitInner);
    }
    if (pmtsd_outer) {
        pmtsd_outer->SetNtotPmts(maxODPMTNo); // JW (2024.02.19.): Set the number of PMTs
...
void CupRootNtuple::SetMuonSD(const G4Event *a_event) {
    ...
    CupVetoHitsCollection *MuHC = (CupVetoHitsCollection *)(HCE->GetHC(MuSDHCID));
    ...
    G4cout << "nTotMuonVeto= " << nTotCell << G4endl;
    G4double eDep;
    ...
    for (int ii = 0; ii<nTotCell; ii++){
        ...
        if (eDep>0.){
            ...
            G4cout << "MuonVeto: ii= " << ii << ", volumeName= " << aHit->GetLogV()->GetName() << ", eDep= " << eDep << ", eDepQuenched= " << eDepQuenched << G4endl;    
        }
        muTotEdep = totalE;
        muTotEdepQuenched = totalEquenched;

        new ((*tclmuon)[ii]) TCell(tcell);
        //new ((*tclmuon)[nCell]) TCell(tcell);
        //nCell++;
    }
    (void)muTotEdep;
    ...
    G4cout << "MuonVeto: nHit= " << iHit << ", TotEdep: " << totalE << G4endl;
