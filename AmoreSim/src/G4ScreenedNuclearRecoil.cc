...
//
// $Id: G4ScreenedNuclearRecoil.cc,v 1.2 2018/07/23 02:39:26 ejjeon Exp $
//
//
// Class Description
...
#include "AmoreSim/G4ScreenedNuclearRecoil.hh"
...
#include "AmoreSim/G4LindhardPartition.hh"
...
void G4ScreenedCoulombCrossSection::BuildMFPTables() {
    ...
    for (G4int matidx = 0; matidx < nMaterials; matidx++) {
        ...
        MFPTables[matidx] =
            c2.log_log_interpolating_function().load(evals, mfpvals, true, 0, true, 0);
    }
}
...
G4ScreenedNuclearRecoil::G4ScreenedNuclearRecoil(const G4String &processName,
    ...
    : G4VDiscreteProcess(processName, fElectromagnetic), screeningKey(ScreeningKey),
      generateRecoils(GenerateRecoils), avoidReactions(1), recoilCutoff(RecoilCutoff),
      physicsCutoff(PhysicsCutoff), hardeningFraction(0.0), hardeningFactor(1.0),
      externalCrossSectionConstructor(0), NIELPartitionFunction(new G4LindhardRobinsonPartition) {
...
G4ScreenedNuclearRecoil::~G4ScreenedNuclearRecoil() { ResetTables(); }
...
G4ScreenedCoulombClassicalKinematics::G4ScreenedCoulombClassicalKinematics()
    : // instantiate all the needed functions statically, so no allocation is
      ...
      phifunc(c2.const_plugin_function()), xovereps(c2.linear(0., 0., 0.)),
...
G4_c2_function &MoliereScreening(G4int z1, G4int z2, size_t npoints, G4double rMax,
    ...
    G4double au =
        0.8853 * 0.529 * angstrom / std::sqrt(std::pow(z1, 0.6667) + std::pow(z2, 0.6667));
    std::vector<G4double> r(npoints), phi(npoints);
    ...
    for (G4int im = 0; im < nMaterials; im++) {
        ...
        for (G4int iEl = 0; iEl < nMatElements; iEl++) {
            ...
            if (sfunciter == phiMap.end()) {
                ...
                G4Exception("G4NativeScreenedCoulombCrossSection::LoadData", "em0003",
                            FatalException, ed);
            }
            ...
            screeningData[Z] = st;
            sigmaMap[Z] =
                c2.log_log_interpolating_function().load(energies, data, true, 0, true, 0);
        }
    }
}
#else
...
G4ParticleDefinition *G4ScreenedCoulombCrossSection::SelectRandomUnweightedTarget(
    ...
    if (!nIsotopes) {
        if (Z <= 92) {
            ...
            nIsotopes = theIso.GetNumberOfIsotopes(Z);
            G4double random =
                100.0 * G4UniformRand(); // values are expressed as percent, sum is 100
            G4int tablestart = theIso.GetFirstIsotope(Z);
            G4double asum    = 0.0;
            for (G4int i = 0; i < nIsotopes; i++) {
                asum += theIso.GetAbundance(i + tablestart);
...
void G4ScreenedCoulombCrossSection::BuildMFPTables() {
    ...
    for (G4int matidx = 0; matidx < nMaterials; matidx++) {
        ...
        G4double logint =
            std::log(emax / emin) / (nmfpvals - 1); // logarithmic increment for tables

        // compute energy scale for interpolator.  Force exact values at both ends to avoid range
        // errors
        for (G4int i = 1; i < nmfpvals - 1; i++)
            evals[i] = emin * std::exp(logint * i);
        evals.front() = emin;
        ...
        for (G4int kel = 0; kel < nMatElements; kel++) {
            ...
            const G4_c2_function &sigma = sigmaMap[Z];
            G4double ndens =
                atomDensities[kel]; // compute atom fraction for this element in this material
    ...
    *cond = NotForced;
    ...
    if (energy < lowEnergyLimit || energy < recoilCutoff * a1) {
        *cond = Forced;
        ...
        energy = highEnergyLimit * a1; /* constant MFP at high energy */
    ...
    // G4cout << "MFP: " << meanFreePath << " index " << materialIndex << " energy " << energy << "
    // MFPScale " << MFPScale << G4endl;
...
G4VParticleChange *G4ScreenedNuclearRecoil::PostStepDoIt(const G4Track &aTrack,
    ...
    if (incidentEnergy < GetRecoilCutoff() * a1) { // check energy sanity on entry
        ...
        // compute the impact parameter very early, so if is rejected as too far away, little effort
        // is wasted this is the TRIM method for determining an impact parameter based on the flight
        // path this gives a cumulative distribution of N(P)= 1-exp(-pi P^2 n l) which says the
        // probability of NOT hitting a disk of area sigma= pi P^2 =exp(-sigma N l) which may be
        // reasonable
        if (sigopi < lattice * lattice) {
            // normal long-flight approximation
            P = std::sqrt(-std::log(G4UniformRand()) * sigopi);
    ...
    if (registerDepositedEnergy) {
        pParticleChange->ProposeLocalEnergyDeposit(IonizingLoss + NIEL);
        pParticleChange->ProposeNonIonizingEnergyDeposit(NIEL);
        // MHM G4cout << "depositing energy, total = " << IonizingLoss+NIEL << " NIEL = " << NIEL <<
        // G4endl;
    }
...
G4bool G4ScreenedCoulombClassicalKinematics::DoScreeningComputation(G4ScreenedNuclearRecoil *master,
    ...
    // numerics note:  because we checked above for reasonable values of beta which give real
    // recoils, we don't have to look too closely for theta -> 0 here (which would cause sin(theta)
    // and 1-cos(theta) to both vanish and make the atan2 ill behaved).
...
void G4NativeScreenedCoulombCrossSection::LoadData(G4String screeningKey, G4int z1, G4double a1,
                                                   G4double recoilCutoff) {
    static const size_t sigLen =
        200; // since sigma doesn't matter much, a very coarse table will do
    G4DataVector energies(sigLen);
    G4DataVector data(sigLen);
    ...
    for (G4int im = 0; im < nMaterials; im++) {
        ...
        for (G4int iEl = 0; iEl < nMatElements; iEl++) {
            ...
            G4double au;
            G4_c2_ptr screen =
                sfunc(z1, Z, 200, 50.0 * angstrom, &au); // generate the screening data
            G4ScreeningTables st;
            ...
            // based on (pi-thetac) = pi*beta*alpha/x0, but noting that alpha is very nearly unity,
            // always so just solve it wth alpha=1, which makes the solution much easier this
            // function returns an approximation to (beta/x0)^2=phi(x0)/(eps*x0)-1 ~
            // ((pi-thetac)/pi)^2 Since we don't need exact sigma values, this is good enough
            // (within a factor of 2 almost always) this rearranges to phi(x0)/(x0*eps) = 2*theta/pi
            // - theta^2/pi^2

            c2_linear_p<G4double> &c2eps =
                c2.linear(0.0, 0.0, 1.0); // will store an appropriate eps inside this in loop
            G4_c2_ptr phiau = screen(c2.linear(0.0, 0.0, au));
            G4_c2_ptr x0func(phiau /
                             c2eps); // this will be phi(x)/(x*eps) when c2eps is correctly set
            x0func->set_domain(1e-6 * angstrom / au,
                               0.9999 * screen->xmax() / au); // needed for inverse function
            // use the c2_inverse_function interface for the root finder... it is more efficient for
            // an ordered computation of values.
            G4_c2_ptr x0_solution(c2.inverse_function(x0func));
            ...
            G4double eratkin =
                0.9999 * (4 * a1 * a2) /
                ((a1 + a2) * (a1 + a2)); // #maximum kinematic ratio possible at 180 degrees
            G4double cmfact0 = st.emin / cm_energy(a1, a2, st.emin);
            G4double l1      = std::log(emax);
            G4double l0      = std::log(st.emin * cmfact0 / eratkin);
            ...
            for (size_t idx = 0; idx < sigLen; idx++) {
                ...
                G4double gamma = 1.0 + ee / m1c2;
                G4double eratio =
                    (cmfact0 * st.emin) / ee; // factor by which ee needs to be reduced to get emin
                G4double theta = thetac(gamma * a1, a2, eratio);

                G4double eps =
                    cm_energy(a1, a2, ee) /
                    escale; // #make sure lab energy is converted to CM for these calculations
                c2eps.reset(0.0, 0.0, eps); // set correct slope in this function

                G4double q = theta / pi;
                // G4cout << ee << " " << m1c2 << " " << gamma << " " << eps << " " << theta << " "
                // << q << G4endl; old way using root finder G4double x0=
                // x0func->find_root(1e-6*angstrom/au, 0.9999*screen.xmax()/au, 1.0, 2*q-q*q); new
                // way using c2_inverse_function which caches useful information so should be a bit
                // faster since we are scanning this in strict order.
                G4double x0 = 0;
                try {
                    x0 = x0_solution(2 * q - q * q);
                } catch (c2_exception e) {
                    // G4Exception(G4String("G4ScreenedNuclearRecoil: failure in inverse solution to
                    // generate MFP Tables: ")+e.what());
                }
                G4double betasquared = x0 * x0 - x0 * phiau(x0) / eps;
                G4double sigma       = pi * betasquared * au * au;
                energies[idx]        = ee;
