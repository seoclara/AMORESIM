...
void CupPosGen_Cosmic::GenerateVertexPositions(G4PrimaryVertex *argVertex, double max_chain_time,
    ...
    if (dist_to_in < kInfinity) {
        ...
        for (G4PrimaryVertex *v = argVertex; v != NULL; v = v->GetNext()) {
            ...
            for (G4PrimaryParticle *pp_tmp = v->GetPrimary(0); pp_tmp != NULL; pp_tmp = pp_tmp->GetNext()) {
                if (abs(pp_tmp->GetPDGcode()) < CupVertexGen_HEPEvt::kPDGcodeModulus)
                    pp_tmp->SetPDGcode(pp_tmp->GetPDGcode() < 0
                                       ? pp_tmp->GetPDGcode() - CupVertexGen_HEPEvt::kPDGcodeModulus
                                       : pp_tmp->GetPDGcode() + CupVertexGen_HEPEvt::kPDGcodeModulus);
            }
        }
    }
