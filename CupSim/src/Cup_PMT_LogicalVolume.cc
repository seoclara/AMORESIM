...
////////////////////////////////////////////////////////////////
// Cup_PMT_LogicalVolume
//
Cup_PMT_LogicalVolume::Cup_PMT_LogicalVolume(
...
static const G4double R3600_z_edge[R3600_n_edge + 1] = {
    188.00, 116.71, 0.00, -116.71, -136.05, -195.00, -282.00, -355.00, -370.00, -422.00};
...
// static const G4double R1408_z_edge[R1408_n_edge+1]= { 75.00, 44.91, 0.00, -44.91, -73.86, -85.00,
// -215.00}; static const G4double R1408_rho_edge[R1408_n_edge+1]= {0.00, 95.91, 102.00,
// 95.91, 44.32, 32.00, 32.00 }; static const G4double R1408_z_o[R1408_n_edge]= {     -51.00,  0.00,
// 0.00, 51.00, -85.00, -215.00 };
//
...
static const G4double R5912_z_o[R5912_n_edge]          = {
    -56.00, 0.00, 0.00, 56.00, -85.00, -215.00 /* includes 20-pin connector at back */};

static const int R7081_n_edge                        = 6;
static const G4double R7081_z_edge[R7081_n_edge + 1] = {96.5, 37.0, 0, -37.0, -68.5, -92.5, -203.5};
static const G4double R7081_rho_edge[R7081_n_edge + 1] = {0,    108.5, 126.5, 108.5,
                                                          58.7, 42.25, 42.25};
static const G4double R7081_z_o[R7081_n_edge]          = {-40.7, 0.0, 0.0, 83.3, -92.5, -203.5};
// static const G4double R7081_z_edge[R7081_n_edge+1]= { 75.00, 53.06, 0.00, -53.06, -73.86, -85.00,
// -215.00}; static const G4double R7081_rho_edge[R7081_n_edge+1]= {0.00, 90.80, 126.50,
// 90.80, 44.32, 42.00, 42.00 }; static const G4double R7081_z_o[R7081_n_edge]= {     -56.00,  0.00,
// 0.00, 56.00, -85.00, -215.00 /* includes 20-pin connector at back */ };

// static const int ETI_9372A_n_edge= 5;
// static const G4double ETI_9372A_z_edge[ETI_9372A_n_edge+1]= { 50.8, 35.56, 0.00, -35.56, -42.42,
// -165.1}; static const G4double ETI_9372A_rho_edge[ETI_9372A_n_edge+1]= {0.00, 45.72, 63.5,
// 45.72, 42.42, 42.42}; static const G4double ETI_9372A_z_o[ETI_9372A_n_edge]= {-25.40,  0.00, 0.00,
// -42.42, -165.1};
static const int ETI_9372A_n_edge                              = 5;
static const G4double ETI_9372A_z_edge[ETI_9372A_n_edge + 1]   = {22.0,   20.0,   0.00,
                                                                -35.56, -42.42, -165.1};
static const G4double ETI_9372A_rho_edge[ETI_9372A_n_edge + 1] = {0,     63.5,  63.5,
                                                                  45.72, 42.42, 42.42};
static const G4double ETI_9372A_z_o[ETI_9372A_n_edge] = {-187.1, 20.0, 0.00, -42.42, -165.1};

////////////////////////////////////////////////////////////////
// Cup_17inch_LogicalVolume
//
Cup_17inch_LogicalVolume::Cup_17inch_LogicalVolume(
    const G4String &plabel,                // label -- subvolume names are derived from this
    G4Material *ExteriorMat,               // material which fills the bounding cylinder
    ...
    : Cup_PMT_LogicalVolume(plabel, 260. * millimeter,
    ...
    if (MaskMat != NULL) {
        ...
        CupTorusStack *facemask_inner_solid = new CupTorusStack(plabel + "_facemask_inner_solid");
        facemask_inner_solid->SetAllParameters(2, z_edge, rho_edge, z_o);

        rho_edge[0] += 2.0 * mm;
        rho_edge[1] += 2.0 * mm;
        rho_edge[2] += 2.0 * mm;
        CupTorusStack *facemask_solid = new CupTorusStack(plabel + "_facemask_solid");
...
//
Cup_20inch_LogicalVolume::Cup_20inch_LogicalVolume(
    const G4String &plabel,                // label -- subvolume names are derived from this
    G4Material *ExteriorMat,               // material which fills the bounding cylinder
    G4Material *GlassMat,                  // glass material
    G4OpticalSurface *Photocathode_opsurf, // photocathode
...
//
Cup_10inch_LogicalVolume::Cup_10inch_LogicalVolume(
    const G4String &plabel,                // label -- subvolume names are derived from this
    G4Material *ExteriorMat,               // material which fills the bounding cylinder
    G4Material *GlassMat,                  // glass material
    G4OpticalSurface *Photocathode_opsurf, // photocathode
    ...
    : Cup_PMT_LogicalVolume(plabel, 130. * millimeter, 155. * millimeter, ExteriorMat) {
    if (PmtStyle == kPmtStyle_Ellipsoid) {
        ConstructPMT_UsingEllipsoid(
    ...
    if (MaskMat != NULL) {
        // make the mask -- use thin cylindrical disk for now
        G4double r_mask_inner = 0; // 96.*millimeter;
        G4double r_mask_outer =
            ((G4Tubs *)(this->GetSolid()))->GetOuterRadius(); // bounding cylinder size
        ...
        new G4PVPlacement(
            0,                                                            // no rotation
            G4ThreeVector(0., 0., R7081_z_edge[1] + hh_mask + z_equator), // displacement
            mask_log,                                                     // logical volume
            plabel + "_mask_phys",                                        // name
            this,                                                         // mother volume
            false,                                                        // no boolean ops
...
//
Cup_8inch_LogicalVolume::Cup_8inch_LogicalVolume(
    const G4String &plabel,                // label -- subvolume names are derived from this
    G4Material *ExteriorMat,               // material which fills the bounding cylinder
    G4Material *GlassMat,                  // glass material
    G4OpticalSurface *Photocathode_opsurf, // photocathode
    ...
    : Cup_PMT_LogicalVolume(plabel, 102. * millimeter, 150. * millimeter, ExteriorMat) {
    if (PmtStyle == kPmtStyle_Ellipsoid) {
        ConstructPMT_UsingEllipsoid(
            102. * millimeter,   // equatorial radius of phototube
            75. * millimeter,    // height of face above eq.
...
//
Cup_5inch_LogicalVolume::Cup_5inch_LogicalVolume(
    const G4String &plabel,                // label -- subvolume names are derived from this
    G4Material *ExteriorMat,               // material which fills the bounding cylinder
    G4Material *GlassMat,                  // glass material
    G4OpticalSurface *Photocathode_opsurf, // photocathode
    ...
    : Cup_PMT_LogicalVolume(plabel, 63.5 * millimeter, 108. * millimeter, ExteriorMat) {
    if (PmtStyle == kPmtStyle_Ellipsoid) {
        ConstructPMT_UsingEllipsoid(
...
// EJ added 2inch PMT
Cup_2inch_LogicalVolume::Cup_2inch_LogicalVolume(
    const G4String &plabel,                // label -- subvolume names are derived from this
    G4Material *ExteriorMat,               // material which fills the bounding cylinder
    G4Material *GlassMat,                  // glass material
    G4OpticalSurface *Photocathode_opsurf, // photocathode
    ...
    : Cup_PMT_LogicalVolume(plabel, 64. * millimeter, 108. * millimeter, ExteriorMat) {
    if (PmtStyle == kPmtStyle_Ellipsoid) {
        ConstructPMT_UsingEllipsoid(
            63.5 * millimeter,   // equatorial radius of phototube
            50.8 * millimeter,   // height of face above eq.
...
//
void Cup_PMT_LogicalVolume::ConstructPMT_UsingTorusStack(
    ...
    CupTorusStack *body_solid = new CupTorusStack(GetName() + "_body_solid");
    body_solid->SetAllParameters(n_edge, outer_z_edge, outer_rho_edge, outer_z_o);

    // inner volumes (to be filled with vacuum)
    CupTorusStack *inner1_solid = new CupTorusStack(GetName() + "_inner1_solid");
    CupTorusStack *inner2_solid = new CupTorusStack(GetName() + "_inner2_solid");
    ...
    G4Tubs *dynode_solid =
        new G4Tubs(GetName() + "_dynode_solid", 0.0, r_dynode, // solid cylinder (fixme?)
...
//
void Cup_PMT_LogicalVolume::ConstructPMT_UsingEllipsoid(
    ...
    if (d_wall > r_stem || d_wall > r_equat) {
        G4cerr << "Error in ConstructPMT_UsingEllipsoid: wall is too thick! "
               << "r_stem=" << r_stem << ", r_equat=" << r_equat << ", d_wall=" << d_wall << G4endl;
        d_wall = (r_stem < r_equat) ? r_stem / 8.0 : r_equat / 8.0;
    }
    if (r_dynode > r_stem - d_wall || r_dynode > r_equat - d_wall) {
        G4cerr << "Error in Cup_ConstructPMT_UsingEllipsoid: dynode is too wide! "
               << "r_dynode=" << r_dynode << ", r_stem=" << r_stem << ", r_equat=" << r_equat
               << ", d_wall=" << d_wall << G4endl;
        r_dynode = r_stem - d_wall;
    }
    if (z_dynode > 0.0) {
        G4cerr << "Error in Cup_ConstructPMT_UsingEllipsoid: dynode extends into face! "
    ...
    CupEllipsoid *face_solid =
        new CupEllipsoid(GetName() + "_face_solid", r_equat, r_equat, h_face, 0., r_equat + h_face);
    CupEllipsoid *back_solid =
        new CupEllipsoid(GetName() + "_back_solid", r_equat, r_equat, -h_back, z_neck, 0.);
    G4Tubs *stem_solid =
        new G4Tubs(GetName() + "_stem_solid", 0.0, r_stem, l_stem_cyl / 2., 0., 2. * M_PI);

    // inner solids
    CupEllipsoid *face_interior_solid =
        new CupEllipsoid(GetName() + "_face_interior_solid", r_equat - d_wall, r_equat - d_wall,
                         h_face - d_wall, 0.0, kInfinity);
    CupEllipsoid *back_interior_solid =
        new CupEllipsoid(GetName() + "_back_interior_solid", r_equat - d_wall, r_equat - d_wall,
                         -h_back - d_wall, z_neck, 0.0);
    G4Tubs *stem_interior_solid =
        new G4Tubs(GetName() + "_stem_interior_solid", 0.0, r_stem - d_wall,
                   l_stem_cyl / 2.0 - d_wall, 0., 2. * M_PI);
    G4Tubs *dynode1_solid;
    G4Tubs *dynode2_solid;
    if (z_dynode < z_neck) {
        dynode1_solid = NULL;
        dynode2_solid = new G4Tubs(GetName() + "_dynode2_solid", 0.0, r_dynode,
                                   (h_back + l_stem + z_dynode - d_wall) / 2.0, 0., 2. * M_PI);
    } else {
        dynode1_solid = new G4Tubs(GetName() + "_dynode1_solid", r_dynode / 2., r_dynode,
                                   (-z_neck + z_dynode) / 2., 0., 2. * M_PI);
        dynode2_solid = new G4Tubs(GetName() + "_dynode2_solid", 0.0, 0.95 * r_dynode,
    ...
    G4LogicalVolume *face_log = new G4LogicalVolume(face_solid, Glass, GetName() + "_face_log");
    face_log->SetSensitiveDetector(detector);
    G4LogicalVolume *back_log = new G4LogicalVolume(back_solid, Glass, GetName() + "_back_log");
    G4LogicalVolume *stem_log = new G4LogicalVolume(stem_solid, Glass, GetName() + "_stem_log");
    G4LogicalVolume *face_interior_log =
        new G4LogicalVolume(face_interior_solid, PMT_Vac, GetName() + "_face_interior_log");
    face_interior_log->SetSensitiveDetector(detector);
    G4LogicalVolume *back_interior_log =
        new G4LogicalVolume(back_interior_solid, PMT_Vac, GetName() + "_back_interior_log");
    G4LogicalVolume *stem_interior_log =
        new G4LogicalVolume(stem_interior_solid, PMT_Vac, GetName() + "_stem_interior_log");
    G4LogicalVolume *dynode1_log =
        (!dynode1_solid)
            ? 0
            : new G4LogicalVolume(dynode1_solid, Dynode_mat, GetName() + "_dynode1_log");
    G4LogicalVolume *dynode2_log =
        new G4LogicalVolume(dynode2_solid, Dynode_mat, GetName() + "_dynode2_log");
    ...
    // calculate z coordinate of equatorial plane in envelope
    z_equator = ((G4Tubs *)(this->GetSolid()))->GetZHalfLength() - h_face -
    ...
    ////
    new G4LogicalBorderSurface(GetName() + "_photocathode_logsurf1", face_interior_phys, face_phys,
                               OpPCSurface);
    new G4LogicalBorderSurface(GetName() + "_photocathode_logsurf2", face_phys, face_interior_phys,
                               OpPCSurface);
    new G4LogicalBorderSurface(GetName() + "_mirror_logsurf1", back_interior_phys, back_phys,
                               our_Mirror_opsurf);
    new G4LogicalBorderSurface(GetName() + "_mirror_logsurf2", back_phys, back_interior_phys,
                               our_Mirror_opsurf);
    ...
    ////
    /**  CupPMTOpticalModel * pmtOpticalModel =  **/
    new CupPMTOpticalModel(GetName() + "_optical_model", face_phys);
